...
void init_interpreter ( void ) {
    ...
    nil       = make_nil();
    sfs_true  = make_boolean( TRUE );
    sfs_false = make_boolean( FALSE );
    LIST_ENV  = create_list_env();
    add_env(create_env(),&LIST_ENV);
    init_primitive();
    uint here2  = 0;
    /*definition de map
    string str = "(define (map proc items) (if (null? items) ‘() (cons (proc (car items)) (map proc (cdr items)))))";
    sfs_eval(sfs_read(str,&here2),LIST_ENV);*/
...
int main ( int argc, char *argv[] ) {
    ...
    while ( 1 ) {
        ...
        output = sfs_eval( sexpr, LIST_ENV);
        if( NULL == output) {
            /* si fichier alors on sort*/
            if (mode == SCRIPT) {
                fclose( fp );
        ...
        sfs_print_first_bracket( output );
        printf( "\n" );
    }
